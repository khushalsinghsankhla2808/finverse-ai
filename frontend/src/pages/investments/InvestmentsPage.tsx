...
export const InvestmentsPage: React.FC = () => {
  ...
  const liveGainPercent =
    liveInvested > 0 ? (liveGain / liveInvested) * 100 : 0;
  ...
  useEffect(() => {
    if (editingInvestment) {
      reset({
        ...
        purchaseDate: new Date(editingInvestment.purchaseDate)
          .toISOString()
          .split("T")[0],
  ...
  const pieData = useMemo(() => {
    ...
    return Object.entries(portfolioSummary.assetAllocation)
      .map(([type, item]: [string, any]) => ({
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-xs text-white/50 font-medium">
              Monitor your asset valuations and returns
            </p>
          </div>
          <Button
            variant="primary"
          ...
          <>
                ...
                <div className="flex flex-wrap gap-2 justify-center max-h-[60px] overflow-y-auto">
                  {pieData.map((p) => (
                    <div key={p.name} className="flex items-center gap-1">
                      ...
                      />
                      <span className="text-[9px] text-white/70 font-semibold truncate">
                        {p.name} ({p.percentage.toFixed(0)}%)
                      </span>
                    </div>
                  ))}
                </div>
                      ...
                      {Object.entries(
                        ...
                        const gain =
                          ...
                          investments
                            ...
                            .reduce((s, i) => s + i.totalInvested, 0);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {investments.map((inv) => {
                ...
                const gainPercent =
                  initialInvested > 0 ? (gain / initialInvested) * 100 : 0;
                    ...
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0 flex flex-col">
                          <h3 className="font-display font-bold text-sm text-white truncate max-w-[120px]">
                            {inv.name}
                          </h3>
                          <span className="text-[10px] text-white/40 font-mono truncate">
                            {inv.symbol || ASSET_LABELS[inv.assetType]}
                          </span>
                        </div>
                      </div>
                    ...
                    <div className="my-5 grid grid-cols-2 gap-4 border-b border-white/5 pb-4">
                      <div>
                        ...
                        <span className="text-sm font-bold font-mono text-white block mt-1">
                          {formatINR(initialInvested)}
                        </span>
                        <span className="text-[10px] text-white/30 font-medium block mt-0.5">
                          {inv.units} units @ {formatINR(inv.purchasePrice)}
                        </span>
                      </div>
                      <div>
                        <span className="text-[9px] text-white/40 uppercase font-bold tracking-wider leading-none">
                        ...
                        <span className="text-sm font-bold font-mono text-white block mt-1">
                          {formatINR(currentTotalValue)}
                        </span>
                        <span className="text-[10px] text-white/30 font-medium block mt-0.5">
                          Price: {formatINR(inv.currentPrice)}
                        </span>
                      </div>
                    </div>
        ...
        >
          <form
            onSubmit={handleSubmit(
              editingInvestment ? onEditSubmit : onAddSubmit,
            )}
            className="space-y-6"
          >
